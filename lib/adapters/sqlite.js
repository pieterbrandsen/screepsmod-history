...
class SqliteAdapter {
  ...
  async read (room, tick, callback) {
    try {
      ...
      if (!record) {
        // Tagged like the file adapter's own error: a caller has to be able to tell an absent
        // chunk from a database that is unwell without matching on the message.
        throw Object.assign(new Error('Record not found'), { code: 'ENOENT' })
      }
